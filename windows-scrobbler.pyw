...
def init_system_tray():
    ...
    tray = SystemTray(menu=menu_def, icon='favicon.ico',
                      tooltip='MediaScrobbler', window=window)
    tray.show_icon()
    return tray
...
async def main():
    ...
    if old_song is not None:
        ...
        if countOldTracks > 0:
            oldMusicBrainzArtist = checkOldTrack.get(
                'recording-list')[0].get('artist-credit')[0].get('artist').get('name')
            oldMusicBrainzTitle = checkOldTrack.get(
                'recording-list')[0].get('title')
            strippedOldSongTitle = old_song.get('title').replace('Facebook', '').replace('Official', '').replace(
                'Music Video', '').replace('YouTube', '').replace('(', '').replace(')', '').replace('-', '').strip()
            comparison = compareSongNames(
                oldMusicBrainzTitle, strippedOldSongTitle)
            if comparison > 0.75:
                network.scrobble(
                ...
                print("\nScrobbled" + oldMusicBrainzArtist + " - " +
                      oldMusicBrainzTitle + "successfully \n\n")
        else:
    ...
    while True:
        ...
        if new_song is not None:
            ...
            if new_song.get('title') != old_song.get('title'):
                old_song = new_song
                strippedNewSongTitle = new_song.get('title').replace('Facebook', '').replace('Official', '').replace(
                    'Music Video', '').replace('YouTube', '').replace('(', '').replace(')', '').replace('-', '').strip()
                ...
                if countSongs > 0:
                    musicBrainzTitle = checkTrack.get(
                        'recording-list')[0].get('title')
                    musicBrainzArtist = checkTrack.get(
                        'recording-list')[0].get('artist-credit')[0].get('artist').get('name')
                    ...
                    if comparison > 0.75:
                        ...
                        print(str(comparison) + " is good enough comparison between " + musicBrainzArtist + "-" +
                              musicBrainzTitle + " and " + new_song.get('artist') + " - " + strippedNewSongTitle + "\n\n")
                        print("\nScrobbled " + musicBrainzArtist +
                              " - " + musicBrainzTitle + " successfully \n\n")
                        # save the scrobbled song in an array
                        scrobbledSongs.append(
                            musicBrainzArtist + " - " + musicBrainzTitle)
                    else:
                        print(str(comparison) + " is not a good enough match for " + musicBrainzArtist + "-" +
                              musicBrainzTitle + " and " + new_song.get('artist') + " - " + strippedNewSongTitle + "\n\n")
                        print(
